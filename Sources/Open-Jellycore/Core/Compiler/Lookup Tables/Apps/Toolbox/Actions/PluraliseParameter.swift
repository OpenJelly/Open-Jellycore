...
struct PluraliseParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "word" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pluralise", name: "word"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "countType" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pluralise", name: "countType"), node: nil)
        }
        if let variableCall = call.first(where: { node in return node.slotName == "items" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pluralise", name: "items"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "count" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pluralise", name: "count"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "output" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pluralise", name: "output"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "typeCase" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pluralise", name: "typeCase"), node: nil)
        }
