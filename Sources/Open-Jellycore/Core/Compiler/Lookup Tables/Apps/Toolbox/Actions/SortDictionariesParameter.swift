...
struct SortDictionariesParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "dictionaries" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "sortDictionaries", name: "dictionaries"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "sortKey" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "sortDictionaries", name: "sortKey"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "sortOrder" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "sortDictionaries", name: "sortOrder"), node: nil)
        }
