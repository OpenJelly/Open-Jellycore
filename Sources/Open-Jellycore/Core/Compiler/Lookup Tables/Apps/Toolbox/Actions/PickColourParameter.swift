...
struct PickColourParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "image" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pickColour", name: "image"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "startingHex" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pickColour", name: "startingHex"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "postURL" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "pickColour", name: "postURL"), node: nil)
        }
