...
struct GetTextFromImagesIAIHParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "images" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getTextFromImagesIAIH", name: "images"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "languageCorrection" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getTextFromImagesIAIH", name: "languageCorrection"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "language" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getTextFromImagesIAIH", name: "language"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "joinText" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getTextFromImagesIAIH", name: "joinText"), node: nil)
        }
