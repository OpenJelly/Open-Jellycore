...
struct GetSymbolNameParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "category" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getSymbolName", name: "category"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "symbolNameType" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getSymbolName", name: "symbolNameType"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "name" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getSymbolName", name: "name"), node: nil)
        }
