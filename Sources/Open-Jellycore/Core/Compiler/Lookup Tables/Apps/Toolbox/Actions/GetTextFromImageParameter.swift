...
struct GetTextFromImageParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "image" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getTextFromImage", name: "image"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "languageCorrection" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getTextFromImage", name: "languageCorrection"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "language" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getTextFromImage", name: "language"), node: nil)
        }
