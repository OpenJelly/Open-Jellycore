...
struct RunShellScriptParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "input" })?.item {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "runShellScript", name: "input"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "script" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "runShellScript", name: "script"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "shell" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "runShellScript", name: "shell"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "inputMode" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "runShellScript", name: "inputMode"), node: nil)
        }
