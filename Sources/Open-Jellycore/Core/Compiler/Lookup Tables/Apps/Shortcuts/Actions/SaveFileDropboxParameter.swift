...
struct SaveFileDropboxParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "file" })?.item {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "saveFileDropbox", name: "file"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "askToSave" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "saveFileDropbox", name: "askToSave"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "path" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "saveFileDropbox", name: "path"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "overwrite" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "saveFileDropbox", name: "overwrite"), node: nil)
        }
