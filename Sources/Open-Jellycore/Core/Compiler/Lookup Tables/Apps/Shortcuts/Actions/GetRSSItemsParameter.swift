...
struct GetRSSItemsParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "url" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getRSSItems", name: "url"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "count" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getRSSItems", name: "count"), node: nil)
        }
