...
struct SetFlashlightParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "state" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "setFlashlight", name: "state"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "level" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "setFlashlight", name: "level"), node: nil)
        }
