...
struct ConditionDetailParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "detail" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "conditionDetail", name: "detail"), node: nil)
        }
        if let variableCall = call.first(where: { node in return node.slotName == "condition" })?.item {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "conditionDetail", name: "condition"), node: nil)
        }
