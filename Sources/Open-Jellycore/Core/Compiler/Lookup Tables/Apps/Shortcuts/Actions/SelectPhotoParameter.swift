...
struct SelectPhotoParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "types" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "selectPhoto", name: "types"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "multiple" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "selectPhoto", name: "multiple"), node: nil)
        }
