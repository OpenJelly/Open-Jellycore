...
struct ProductDetailParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "detail" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "productDetail", name: "detail"), node: nil)
        }
        if let variableCall = call.first(where: { node in return node.slotName == "product" })?.item {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "productDetail", name: "product"), node: nil)
        }
