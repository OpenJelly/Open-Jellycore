...
struct CombineImageParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "images" })?.item {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "combineImage", name: "images"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "mode" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "combineImage", name: "mode"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "spacing" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "combineImage", name: "spacing"), node: nil)
        }
