...
struct OpenInParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "input" })?.item {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "openIn", name: "input"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "app" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "openIn", name: "app"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "ask" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "openIn", name: "ask"), node: nil)
        }
