...
struct ContactDetailParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "detail" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "contactDetail", name: "detail"), node: nil)
        }
        if let variableCall = call.first(where: { node in return node.slotName == "contact" })?.item {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "contactDetail", name: "contact"), node: nil)
        }
