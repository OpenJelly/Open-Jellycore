...
struct ReplaceTextParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "input" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "replaceText", name: "input"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "find" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "replaceText", name: "find"), node: nil)
        }
