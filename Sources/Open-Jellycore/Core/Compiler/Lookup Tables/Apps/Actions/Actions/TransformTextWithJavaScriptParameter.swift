...
struct TransformTextWithJavaScriptParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "text" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "transformTextWithJavaScript", name: "text"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "javaScriptCode" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "transformTextWithJavaScript", name: "javaScriptCode"), node: nil)
        }
