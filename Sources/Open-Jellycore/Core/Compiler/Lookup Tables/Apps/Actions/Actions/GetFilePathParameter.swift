...
struct GetFilePathParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let variableCall = call.first(where: { node in return node.slotName == "file" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getFilePath", name: "file"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "type" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "getFilePath", name: "type"), node: nil)
        }
