...
struct EncodeHexParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "operation" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "encodeHex", name: "operation"), node: nil)
        }
        if let variableCall = call.first(where: { node in return node.slotName == "input" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "encodeHex", name: "input"), node: nil)
        }
