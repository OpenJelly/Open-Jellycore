...
struct EncodePlistParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "operation" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "encodePlist", name: "operation"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "format" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "encodePlist", name: "format"), node: nil)
        }
        if let variableCall = call.first(where: { node in return node.slotName == "input" }) {
            ...
            EventReporter.shared.reportError(error: .missingParameter(function: "encodePlist", name: "input"), node: nil)
        }
