//
//  aShellMini_ExecuteCommandParameter.swift
//  Open-Jellycore
//
//  Created by Taylor Lineman on 6/02/23.
//

struct aShellMini_ExecuteCommandParameter: ParameterProtocol, Codable {
	var command: JellyArray<JellyVariableReference>?
	var openWindow: aShellMini_Jelly_Enum?
	var keepGoing: JellyBoolean?


    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        var parameters = aShellMini_ExecuteCommandParameter()
        ...
        if let value = call.first(where: { node in return node.slotName == "openWindow" }) {
            parameters.openWindow = aShellMini_Jelly_Enum(parameterItem: value, scopedVariables: scopedVariables)
        } else {
