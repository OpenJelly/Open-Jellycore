...
struct ImageFromPDFParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "format" }) {
            parameters.WFMakeImageFromPDFPageImageFormat = Jelly_WFMakeImageFromPDFPageImageFormat(value, scopedVariables: scopedVariables)
        } else {
        ...
        if let value = call.first(where: { node in return node.slotName == "colorspace" }) {
            parameters.colorspace = Jelly_WFMakeImageFromPDFPageColorspace(value, scopedVariables: scopedVariables)
        } else {
        ...
        if let value = call.first(where: { node in return node.slotName == "resolution" }) {
            parameters.WFMakeImageFromPDFPageResolution = JellyDouble(value, scopedVariables: scopedVariables)
        } else {
