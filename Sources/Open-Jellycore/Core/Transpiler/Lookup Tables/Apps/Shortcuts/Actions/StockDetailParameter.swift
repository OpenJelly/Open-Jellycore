...
struct StockDetailParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "detail" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "stockDetail", name: "detail"), node: nil)
        }
        if let variableCall = call.first(where: { node in return node.slotName == "stock" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "stockDetail", name: "stock"), node: nil)
        }
