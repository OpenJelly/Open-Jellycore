...
struct FindArtistsParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "searchMode" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "findArtists", name: "searchMode"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "query" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "findArtists", name: "query"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "ids" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "findArtists", name: "ids"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "limitResults" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "findArtists", name: "limitResults"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "limitCount" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "findArtists", name: "limitCount"), node: nil)
        }
