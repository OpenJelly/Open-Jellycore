...
struct DecreaseNumberTrackerParameter: ParameterProtocol, Codable {
    ...
    static func build(call: [FunctionCallParameterItem], scopedVariables: [Variable]) -> ParameterProtocol {
        ...
        if let value = call.first(where: { node in return node.slotName == "tracker" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "decreaseNumberTracker", name: "tracker"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "amount" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "decreaseNumberTracker", name: "amount"), node: nil)
        }
        if let value = call.first(where: { node in return node.slotName == "note" }) {
            ...
            ErrorReporter.shared.reportError(error: .missingParameter(function: "decreaseNumberTracker", name: "note"), node: nil)
        }
